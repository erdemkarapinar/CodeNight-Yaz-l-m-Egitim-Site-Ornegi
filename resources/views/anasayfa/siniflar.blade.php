        ...
        <header class="sticky-top" style="background-color: yellow; box-shadow: 5px 5px 5px #999;">
            <div class="container">
              <div class="d-flex flex-wrap align-items-center justify-content-center justify-content-lg-start">
               ...
               <section class="md-4" style="margin: 2px; background-color: #2F4F4F;">
            <div class="container">
                <div class="row">      
                	<h3 style="color: white;">Yazılım Sınıfları</h3>
                    @foreach($y as $siniflar => $sinif)
                        @php
                            // Sabit resim ve açıklama dizilerinden uygun olanları seç
                            $image = $images[$siniflar % count($images)];
                            $description = $descriptions[$siniflar % count($descriptions)];
                        @endphp
                        <div class="col-md-6">
                            <div class="card" style="margin: 5px; box-shadow: 2px -1px 5px 2px #999">
                                    <div class="card-body">
                                        <h5 class="card-title">{{ $sinif->sinif }}</h5>
                                        <img class="card-image" src="{{ $image }}" alt="{{ $sinif->sinif }}" style="box-shadow: 5px 2px 5px 2px #999; width: 300px; height: 300px;">
                                        <p class="card-text">
                                            {{ $descriptions[$siniflar % count($descriptions)] }}
                                        </p>
                                        <a href="" class="btn btn-primary">Sınıfa Göz At</a>
                                    </div>
                            </div> 
                        </div>
